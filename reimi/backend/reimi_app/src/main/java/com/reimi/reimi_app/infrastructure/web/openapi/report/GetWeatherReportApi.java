...
@Operation(
    summary = "ウェザーリポート一覧取得",
    description = "今日投稿されているウェザーリポートの一覧を取得できるAPI"
)
@ApiResponses({
    @ApiResponse(
        responseCode = "200",
        description = "ウェザーリポートの一覧取得成功",
        content = @Content(
            mediaType = "application/json",
            array = @ArraySchema(
                schema = @Schema(implementation = GetWeatherReportListResponse.class)
            )
        )
    ),
    @ApiResponse(
...
public @interface GetWeatherReportApi {
}